...
class GraphLine{
  ...
  public:
    ...
    void draw( Adafruit_SSD1680& disp, int yGraph, int heightGraph, int minY, int maxY) {
     ...
     */ 
    static void drawSymbol( Adafruit_SSD1680& disp, int x, int y, int size, PointSymbol symbol){
      ...
      switch(symbol) {
         case CIRCLE:
         {
            disp.fillCircle(x, y, halfSize, EPD_RED); // PICO, 2 was halfsize
            break; 
         }
         case SQUARE:
         {
            ...
            disp.drawRect(topLeftX, topLeftY, size, size, EPD_BLACK);
            break; 
         }
         case TRIANGLE:
         {
            ...
            disp.drawTriangle(x0, y0, x1, y1, x2, y2, EPD_BLACK);
            break;
         }
         case POINT:
         {
            disp.drawPixel(x, y, EPD_BLACK);
            break;
         }
         default:
         {
...
class ScrollingLineGraphMultiValue{
  protected:
    ...
    int _widthGraph = 250; // in pixels
    int _heightGraph = 122; // in pixels

    boolean _drawAxis = true;
    boolean _isAutoYAxis = false;
  ...
  public:
    ...
    ScrollingLineGraphMultiValue(int numLines, PointSymbol symbols[]) :
      ScrollingLineGraphMultiValue(0, 0, 250, 122, numLines, symbols, DEFAULT_SYMBOL_SIZE){
        // purposefully empty
      }

    ScrollingLineGraphMultiValue(int numLines, PointSymbol symbols[], int symbolSize) :
      ScrollingLineGraphMultiValue(0, 0, 250, 122, numLines, symbols, symbolSize){
        // purposefully empty
      }
    ...
    void draw( Adafruit_SSD1680& disp){
      if(_drawAxis){
        drawAxis(disp);
      }
    ...
    // Draw the axis
    void drawAxis( Adafruit_SSD1680& disp){
      int16_t xText, yText;
      ...
      for(int x = getLeft(); x < getRight(); x += 5){
        disp.drawPixel(x, yPos, EPD_BLACK);
      }
    ...
    void drawLegend( Adafruit_SSD1680& disp, int x, int y){
      int xCurPos = -1;
      int ySymbolPos = -1;
      int16_t xText, yText;
      uint16_t textWidth, textHeight;
